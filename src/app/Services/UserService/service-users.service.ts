...
export class ServiceUsersService {
  ...
  ChangeCurrentpassRequest(a:ChangePasswordRequest){
    ...
    const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
    return this.http.put('http://localhost:8080/change/change-current-pass',a, { headers });
  }
getConnectedUser(){
  ...
  const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
  return this.http.get('http://localhost:8080/email/getconnecteduser', { headers });
}
  ...
  searchUser(name:String):any {
    ...
    const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
      return this.http.get<User[]>('http://localhost:8080/user/searchUser/'+name, { headers });
  }
